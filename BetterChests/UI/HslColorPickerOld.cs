...
/// </summary>
internal class HslColorPickerOld : DiscreteColorPicker
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="HslColorPickerOld" /> class.
    /// </summary>
    ...
    public HslColorPickerOld(IModHelper helper, int x, int y, Item? item = default)
        : base(x, y, 0, item)
    {
        this.Helper = helper;
        this.width = HslColorPickerOld.Width;
        this.height = HslColorPickerOld.Height;

        var barWidth = this.width / 2 - HslColorPickerOld.Gap;
        var barHeight = (this.height - HslColorPickerOld.Gap - 36) / 2;
        var cellSize = barHeight / HslColorPickerOld.Cells;
        ...
        this.LightnessBar = Enumerable.Range(0, HslColorPickerOld.Cells).Select(
            i => new Rectangle(centerX + HslColorPickerOld.Gap / 2, top + i * cellSize, barWidth, cellSize)).ToList();
        this.SaturationBar = Enumerable.Range(0, HslColorPickerOld.Cells).Select(
            i => new Rectangle(
                centerX + HslColorPickerOld.Gap / 2,
                top + barHeight + HslColorPickerOld.Gap + i * cellSize,
                barWidth,
                cellSize)).ToList();
        ...
        this.NoColorButton = new(
            new(this.xPositionOnScreen - 2, this.yPositionOnScreen, 7, 7),
            Game1.mouseCursors,
            new(295, 503, 7, 7),
            Game1.pixelZoom);
    ...
    private HslColor[] ColorsHsl => HslColorPickerOld.CachedColorsHsl ??=
        this.ColorsRgb.Select(HslColor.FromColor).Distinct().ToArray();

    private Color[] ColorsRgb
    {
        get
        {
            if (HslColorPickerOld.CachedColorsRgb is not null)
            {
                return HslColorPickerOld.CachedColorsRgb;
            }

            HslColorPickerOld.CachedColorsRgb = new Color[this.HueBar.Width * this.HueBar.Height];
            this.HueBar.GetData(HslColorPickerOld.CachedColorsRgb);
            return HslColorPickerOld.CachedColorsRgb;
        }
    }
    ...
    private Range<int> HslTrack => HslColorPickerOld.CachedHslTrack ??= new(0, this.ColorsHsl.Length - 1);

    private Texture2D HueBar =>
        HslColorPickerOld.CachedHueBar ??= this.Helper.ModContent.Load<Texture2D>("assets/hue.png");
    ...
    private int HueCoord
    {
        ...
        set
        {
            ...
            var hueIndex = this.HslTrack.Clamp(
                this._hueCoord.Remap(this.HueTrack, HslColorPickerOld.UnitRange)
                    .Remap(HslColorPickerOld.UnitRange, this.HslTrack));
            var hslColor = this.ColorsHsl[hueIndex];
            this._hslColor.H = hslColor.H;
            if (this.IsBlack)
            {
                this.SaturationCoord = hslColor.S.Remap(HslColorPickerOld.UnitRange, this.SaturationTrack);
                this.LightnessCoord = hslColor.L.Remap(HslColorPickerOld.UnitRange, this.LightnessTrack);
            }
    ...
    private int LightnessCoord
    {
        ...
        set
        {
            ...
            this._hslColor.L =
                HslColorPickerOld.UnitRange.Clamp(value.Remap(this.LightnessTrack, HslColorPickerOld.UnitRange));
            this.IsBlack = false;
        }
    }
    ...
    private int SaturationCoord
    {
        ...
        set
        {
            ...
            this._hslColor.S =
                HslColorPickerOld.UnitRange.Clamp(value.Remap(this.SaturationTrack, HslColorPickerOld.UnitRange));
            this.IsBlack = false;
        }
    }
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        // Menu Background
        HslColorPickerOld.drawTextureBox(
            b,
            this.xPositionOnScreen - HslColorPickerOld.borderWidth / 2,
            this.yPositionOnScreen - HslColorPickerOld.borderWidth / 2,
            this.width + HslColorPickerOld.borderWidth,
            this.height + HslColorPickerOld.borderWidth,
            Color.LightGray);
        ...
        // Saturation Bar
        foreach (var (bar, color) in this.SaturationBar.Select(
                     (bar, index) => (bar, this.GetSaturationShade((float)index / HslColorPickerOld.Cells))))
        {
            b.Draw(Game1.staminaRect, bar, color);
        }

        // Lightness Bar
        foreach (var (bar, color) in this.LightnessBar.Select(
                     (bar, index) => (bar, this.GetLightnessShade((float)index / HslColorPickerOld.Cells))))
        {
            b.Draw(Game1.staminaRect, bar, color);
        }
        ...
        if (this.itemToDrawColored is Chest chest)
        {
            chest.draw(
                ...
                this.yPositionOnScreen - HslColorPickerOld.borderWidth / 2 - Game1.tileSize,
                1f,
                true);
        }
    }
    ...
    public override void receiveScrollWheelAction(int direction)
    {
        ...
        this.colorSelection = HslColorPickerOld.GetSelectionFromColor(color);
        if (this.itemToDrawColored is Chest chest)
        {
    ...
    public override void update(GameTime time)
    {
        ...
        if (this.ColorSelection != this.colorSelection)
        {
            ...
            {
                var initHsl = HslColor.FromColor(HslColorPickerOld.GetColorFromSelection(this.colorSelection));
                for (var coord = this.HueBarArea.Top; coord <= this.HueBarArea.Bottom; coord++)
                {
                    if (Math.Abs(
                            initHsl.H
                          - this.ColorsHsl[this.HslTrack.Clamp(
                                               coord.Remap(this.HueTrack, HslColorPickerOld.UnitRange).Remap(
                                                   HslColorPickerOld.UnitRange,
                                                   this.HslTrack))].H)
                      < 0.001)
                    {
                        this.HueCoord = coord;
                ...
                this.SaturationCoord = initHsl.S.Remap(HslColorPickerOld.UnitRange, this.SaturationTrack);
                this.LightnessCoord = initHsl.L.Remap(HslColorPickerOld.UnitRange, this.LightnessTrack);
            }
            else
            {
                this.HueCoord = 0;
        ...
        var color = this.GetCurrentColor();
        this.ColorSelection = this.colorSelection = HslColorPickerOld.GetSelectionFromColor(color);
        if (this.itemToDrawColored is Chest chest)
        {
            chest.playerChoiceColor.Value = color;
            chest.resetLidFrame();
    ...
    {
        return new HslColor { H = this._hslColor.H, S = this._hslColor.S, L = value }.ToRgbColor();
    }

    private Color GetSaturationShade(float value)
    {
        return new HslColor { H = this._hslColor.H, S = value, L = Math.Max(0.01f, this._hslColor.L) }.ToRgbColor();
    }
}