...
//     public StorageManager(ModConfig config)
//     {
//         // Init
...
//
//         this.config.VanillaStorages.TryAdd("Chest", new StorageData());
//         this.config.VanillaStorages.TryAdd("Fridge", new StorageData { CustomColorPicker = FeatureOption.Disabled });
//         this.config.VanillaStorages.TryAdd(
//             "Junimo Chest",
//             new StorageData { CustomColorPicker = FeatureOption.Disabled });
//
...
//             new StorageData { CustomColorPicker = FeatureOption.Disabled });
//
//         this.config.VanillaStorages.TryAdd("Stone Chest", new StorageData());
//
//         // Events
...
//
//             // Iterate Inventory
//             foreach (var storage in StorageManager.FromPlayer(Game1.player, excluded))
//             {
...
//                 });
//
//             foreach (var storage in locations.SelectMany(location => StorageManager.FromLocation(location, excluded)))
//             {
//                 storages.Add(storage);
//                 yield return storage;
//             }
...
//                 }
//
//                 foreach (var subStorage in StorageManager.FromStorage(storageObject, excluded))
//                 {
...
//         Game1.player.CurrentItem is not null && StorageManager.TryGetOne(Game1.player.CurrentItem, out var storage)
//             ? storage
//             : null;
//
//     /// <summary>Gets all placed storages in the current location.</summary>
//     public static IEnumerable<StorageNode> CurrentLocation => StorageManager.FromLocation(Game1.currentLocation);
//
//     /// <summary>Gets storages in the farmer's inventory.</summary>
//     public static IEnumerable<StorageNode> Inventory => StorageManager.FromPlayer(Game1.player);
//
//     private static ModConfig Config => StorageManager.instance.config;
//
...
//         {
//             yield return StorageManager.GetStorageType(storage);
//         }
//
//         // Get Fridge
...
//             yield return StorageManager.GetStorageType(new FridgeStorage(location, fridgePosition.ToVector2()));
//         }
//
//         // Get Shipping Bin
...
//             yield return StorageManager.GetStorageType(
...
//                                || !StorageManager.TryGetOne(obj, location, position, out var subStorage)
...
//             yield return StorageManager.GetStorageType(subStorage);
//         }
//     }
//
...
//             if (!StorageManager.TryGetOne(item, player, position, out var storage)
...
//             || !StorageManager.TryGetOne(obj, location, pos, out var storageObject))
//         {
//             storage = default;
//             return false;
//         }
//
//         storage = StorageManager.GetStorageType(storageObject);
//         return true;
//     }
//
...
//                 storage = StorageManager.GetStorageType(baseStorage);
//                 return true;
//         }
//
//         if (!IntegrationsManager.TryGetOne(context, out var storageObject)
//             && !StorageManager.TryGetOne(context, default, default, out storageObject))
//         {
//             storage = default;
//             return false;
//         }
...
//     {
//         excluded ??= new HashSet<object>();
//         if (!excluded.Add(storage.Context))
//         {
...
//         {
//             if (!StorageManager.TryGetOne(item, storage.Source, storage.Position, out var storageObject)
...
//         }
//
//         return StorageManager.GetStorageTypes(storages).Concat(
//             storages.SelectMany(subStorage => StorageManager.FromStorage(subStorage, excluded)));
//     }
//
...
//         StorageManager.instance.storageTypeRequested.InvokeAll(StorageManager.instance, storageTypeRequestedEventArgs);
//         var storageType = storageTypes.FirstOrDefault();
//         return new StorageNode(
//             storage,
//             storageType is not null ? new StorageNode(storageType, StorageManager.Config) : StorageManager.Config);
//     }
//
//     private static IEnumerable<StorageNode> GetStorageTypes(IEnumerable<Storage> storages) =>
//         storages.Select(StorageManager.GetStorageType);
//
//     private static bool TryGetOne(object? context,
//         object? parent,
//         Vector2 position,
